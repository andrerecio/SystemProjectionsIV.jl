...
function _weakivtest_core(
    ...
    sebtsls_val = sqrt((1 / pipi) * dot(pihat, S_1tsls * pihat) * (1 / pipi) / S)
    ...
    if is_robust
        ...
        sebliml_val = sqrt((1 / limlden) * dot(pihat, S_1liml * pihat) * (1 / limlden) / S)
    else
...
function _compute_BLIML_nagar(W_1::AbstractMatrix{T}, W_12, W_2, Omega, eps::T) where {T}
    ...
    betastart = _find_betastart(beta -> _BmaxLIML(beta, W_1, W_12, W_2, Omega), LimitB, eps)

    if betastart == 0
        return _BmaxLIML(T(0), W_1, W_12, W_2, Omega)
    end

    best_beta = _grid_search(beta -> _BmaxLIML(beta, W_1, W_12, W_2, Omega), betastart)

    best_beta = _nelder_mead_1d(beta -> -_BmaxLIML(beta, W_1, W_12, W_2, Omega), best_beta)
...
function _find_betastart(f, LimitB, eps)
    ...
    betastart = 0
    val = max(abs(abs(f(betastart)) / LimitB - 1), abs(abs(f(-betastart)) / LimitB - 1))
