...
using StatsAPI:
    ...
    vcov,
    weights
using StatsBase: StatsBase, AbstractWeights, CoefTable, UnitWeights, Weights, mean, uweights
...
function esample(m::Union{OLSEstimator,IVEstimator}, v::AbstractVector)
    length(v) == length(m.esample) || throw(
        ArgumentError(
            "Vector length ($(length(v))) must match original data length ($(length(m.esample)))",
        ),
    )
    return v[m.esample]
end
...
@compile_workload begin
    df = DataFrame(
        ...
        x2 = [1.0, 2.0, 4.0, 4.0, 3.0, 5.0],
        y = [3.0, 4.0, 4.0, 5.0, 1.0, 2.0],
        id = [1, 1, 2, 2, 3, 3],
        z = [2.0, 3.0, 1.0, 4.0, 2.0, 5.0],
    )  # instrument
