...
function ols(
    ...
    # Validate keywords
    factorization in (:auto, :chol, :qr) || throw(
        ArgumentError("factorization must be :auto, :chol, or :qr, got :$factorization"),
    )
...
function iv(
    ...
    n_endogenous <= size(X, 2) || throw(
        ArgumentError("n_endogenous ($n_endogenous) > number of X columns ($(size(X,2)))"),
    )
