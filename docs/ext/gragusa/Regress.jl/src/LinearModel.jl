...
function StatsAPI.response(m::OLSMatrixEstimator)
    has_response_data(m.rr) || error("Response vector not stored.")
    return m.rr.y
end

function StatsAPI.fitted(m::OLSMatrixEstimator)
    has_response_data(m.rr) || error("Fitted values not stored.")
    return m.rr.mu
end

function StatsAPI.modelmatrix(m::OLSMatrixEstimator)
    has_predictor_data(m.pp) || error("Model matrix not stored.")
    return m.pp.X
end
...
function StatsAPI.residuals(m::OLSEstimator)
    has_response_data(m.rr) || error(
        "Response vector not stored. Model was fit with save=:minimal. Cannot compute residuals.",
    )
    resid = m.rr.y .- m.rr.mu
    # Unweight residuals if model was estimated with weights
    if isweighted(m.rr)
...
function _predict_ols_impl(
    ...
    if has_fe(m)
        ...
        fes = leftjoin(
            select(df, m.fes.fe_names),
            dropmissing(unique(m.fes.fe));
            on = m.fes.fe_names,
            makeunique = true,
            matchmissing = :equal,
            order = :left
...
function StatsAPI.residuals(m::OLSEstimator{T}, data) where {T}
    Tables.istable(data) ||
        throw(ArgumentError("expected second argument to be a Table, got $(typeof(data))"))
    has_fe(m) && throw(
        "To access residuals for a model with high-dimensional fixed effects, access them directly with `residuals(m)`.",
    )
