...
function CovarianceMatrices.momentmatrix(m::IVEstimator)
    ...
    !has_residuals_data(m) && error(
        "Model does not have residuals stored. Use save=:residuals or save=:all when fitting.",
    )
...
function Base.show(io::IO, ::MIME"text/html", m::IVEstimator)
    ...
    html_row(
        io,
        [
            "Note: Std. errors computed using $vcov_name variance estimator",
            "",
            "",
            "",
            "",
...
function StatsAPI.residuals(m::IVEstimator, data)
    Tables.istable(data) ||
        throw(ArgumentError("expected second argument to be a Table, got $(typeof(data))"))
    has_fe(m) && throw(
        "To access residuals for a model with high-dimensional fixed effects,  run `m = iv(..., save = :residuals)` and then access residuals with `residuals(m)`.",
    )
    cdata = Tables.columntable(data)
    cols, nonmissings = StatsModels.missing_omit(cdata, m.formula_schema.rhs)
    Xnew = modelmatrix(m.formula_schema, cols)
...
function StatsAPI.residuals(m::IVEstimator{T}) where {T}
    if !has_residuals_data(m)
        has_fe(m) && throw(
            "To access residuals in a fixed effect regression, run `iv` with the option save = :residuals, and then access residuals with `residuals()`",
        )
        !has_fe(m) && throw(
            "To access residuals, use residuals(m, data) where `m` is an estimated IVEstimator and `data` is a Table",
        )
    end
