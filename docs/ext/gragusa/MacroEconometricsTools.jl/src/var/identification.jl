...
function identify_sign_distributed(
    ...
    if verbose
        println(
            "Searching for valid rotation using $active_workers workers (draw allocations: $(join(draws_per_worker, ", ")))...",
        )
    end
    ...
    for (idx, result) in enumerate(results)
        ...
        if result.found
            if verbose
                println(
                    "Worker $worker_id found valid rotation at attempt $(result.attempt)",
                )
            end
            return result.P
        end
