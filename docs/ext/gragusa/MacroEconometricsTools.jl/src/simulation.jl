...
function simulate_var!(
    ...
    size(Y_sim, 1) == n_total ||
        throw(ArgumentError("Y_sim must have $n_total rows (got $(size(Y_sim, 1)))"))
    size(Y_sim, 2) == n_vars_val ||
        throw(ArgumentError("Y_sim must have $n_vars_val columns (got $(size(Y_sim, 2)))"))
