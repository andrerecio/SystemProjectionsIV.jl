...
# Trait: does the spec consume the LP controls (X / intercept / xlags)? The VAR
# variant ignores them — its own lags play that role.
_uses_controls(::SPIVwithLP) = true
_uses_controls(::SPIVwithVAR) = false

# Number of leading burn-in rows to drop: the sample starts at the first row that is
# NaN-free across every input (lag/lags pad initial rows with NaN; any other
# non-finite value is a data error, rejected by the caller). Returns T when no row
# is NaN-free.
function _burnin_rows(yv::AbstractVector{Float64}, Ms::AbstractMatrix{Float64}...)
    for t in eachindex(yv)
        if !isnan(yv[t]) && all(M -> all(j -> !isnan(M[t, j]), axes(M, 2)), Ms)
            return t - 1
        end
    end
    return length(yv)
...
"""
    spiv(y, Y, Z, spec = SPIVwithLP(); H, X = nothing, intercept = true, xlags = 0,
         p = 1, weak_iv = :AR, α = 0.05, ξ = 0.10, grid_length = 30, grid_scale = 10,
         hac = :fixed)
...
spiv(y, Y, z; H = 8, xlags = 4)            # + 4 lags of [y Y z] as controls
spiv(y, Y, z; H = 8, X = lags(w, 4))       # + user-built lagged controls
...
- `SPIVwithVAR()` — a VAR(`p`) on `[Y_t, y_t, z_t]`; forecast errors are built from the
  reduced-form residuals (eqs. A.2–A.3) and the controls (`X`, `intercept`, `xlags`)
  are **ignored with a warning** (the VAR's own lags play that role).
  `T_eff = T − p − (H − 1)`.
...
**Sample adjustment.** Leading rows containing `NaN` in any input — the burn-in
that [`lag`](@ref) / [`lags`](@ref) produce — are dropped from *all* inputs
automatically (Stata/R-style), so `X = lags(w, 4)` works as-is; the adjusted sample
is visible as `T_eff` in the printed result. Any other non-finite value — `Inf`
anywhere, or `NaN` after the burn-in — is a data error and throws an
`ArgumentError`.

Keyword arguments:

- `X` — additional controls to residualise on (e.g. lagged variables, `X_{t-1}`),
  a `T × Nx` matrix or a length-`T` vector; default none.
- `intercept` — prepend a constant column to the controls (default `true`).
- `xlags` — also control for this many lags of `[y Y Z]` (in that column order),
  built and sample-adjusted automatically (default 0). With `xlags = p`, the LP
  variant conditions on the same information set as `SPIVwithVAR()` of lag order `p`.
...
`result.irf_endogenous` (docs/technical.md §3.3, eq. 11), or via [`irf`](@ref).
"""
function spiv(
    ...
    # --- keyword and dimension validation ----------------------------------
    H ≥ 1 || throw(ArgumentError("H must be ≥ 1, got $H"))
    p ≥ 1 || throw(ArgumentError("p must be ≥ 1, got $p"))
    xlags ≥ 0 || throw(ArgumentError("xlags must be ≥ 0, got $xlags"))
    ...
    # --- controls: only the LP variant consumes them ------------------------
    if _uses_controls(spec)
        if xlags ≥ 1
            xlags < T ||
                throw(ArgumentError("xlags = $xlags leaves no observations (T = $T)"))
            L = lags(hcat(yv, Ym, Zm), xlags)
            Xuser = size(Xuser, 2) == 0 ? L : hcat(Xuser, L)
        end
    else
        if X !== nothing || !intercept || xlags != 0
            @warn "X, intercept, and xlags are ignored with SPIVwithVAR: the VAR's " *
                  "own lags replace the LP controls"
        end
        Xuser = Matrix{Float64}(undef, T, 0)   # the VAR variant never uses controls
    end

    # --- automatic sample adjustment: drop the leading NaN burn-in rows ----
    # (the padding that lag/lags produce). Any other non-finite value — Inf
    # anywhere, NaN after the burn-in — is a data error, rejected below.
    r = _burnin_rows(yv, Ym, Zm, Xuser)
    r == T && T > 0 &&
        throw(
            ArgumentError(
            "the leading NaN burn-in spans all $T rows — no observations left to estimate"),
        )
    if r > 0
        keep = (r + 1):T
        yv = yv[keep]
    ...
    for (name, arr) in (("y", yv), ("Y", Ym), ("Z", Zm), ("X", Xuser))
        all(isfinite, arr) || throw(
            ArgumentError(
            "$name contains non-finite values (NaN/Inf) beyond the leading NaN " *
            "burn-in (dropped automatically) — these are data errors, not lag padding."),
        )
    end
